...
class Router {
    ...
    public function run ( ){
        if ( $match = $this -> match( ) ) {
            ...
            foreach ( $reflectionController -> getAttributes( inject::class ) as $injectAttribute ) foreach ( $injectAttribute -> newInstance( ) -> injects( ) as $Attributekey => $Attributevalue ) $class -> $Attributekey = $Attributevalue ;
            foreach ( $reflectionController -> getMethod( $match[ 'method' ] ) -> getAttributes( inject::class ) as $injectParams ) foreach ( $injectParams -> newInstance( ) -> injects( ) as $paramskey => $paramsvalue ) $match[ 'params' ][ $paramskey ] = $paramsvalue ;
